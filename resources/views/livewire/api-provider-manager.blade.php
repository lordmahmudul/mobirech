<x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('API Providers') }}</h2>
</x-slot>
...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
            ...
            <x-button wire:click="create" class="mb-4">{{ __('Add Provider') }}</x-button>

            <table class="min-w-full border-collapse block md:table">
                <thead class="block md:table-header-group">
                    <tr class="bg-gray-100">
                        <th class="p-2 text-left font-bold">Name</th>
                        <th class="p-2 text-left font-bold">Current Balance</th>
                ...
                <tbody class="block md:table-row-group">
                    ...
                    <tr class="bg-white border-b">
                        ...
                        <td class="p-2">{{ number_format($provider->current_balance, 2) }}</td>
            ...
            <x-dialog-modal wire:model="isModalOpen">
                <x-slot name="title">{{ $provider_id ? 'Edit Provider' : 'Add Provider' }}</x-slot>
                <x-slot name="content">
                    <div class="mb-4">
                        <x-label for="provider_name" value="Provider Name" />
                    ...
                    <div class="mb-4">
                        ...
                        <x-input id="current_balance" type="number" step="0.01" class="block w-full mt-1" wire:model="current_balance" />
                        @error('current_balance') <span class="text-red-500">{{ $message }}</span> @enderror
                    </div>
                </x-slot>
                <x-slot name="footer">
